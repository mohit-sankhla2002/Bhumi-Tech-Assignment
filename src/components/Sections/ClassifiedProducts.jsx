...
function ClassifiedProducts() {
  return (
    <div className='bg-lightGray px-section py-10 flex items-center gap-10'>
      ...
      >
        {classifiedProducts.map((product, index) => <SwiperSlide key={index}><ClassifiedProduct img={product.img} title={product.name} price={product.price} location={product.location} /></SwiperSlide>)}
        </Swiper>
    </div>
  )
}
